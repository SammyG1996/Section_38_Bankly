...
describe("GET /users", function() {
  ...
  // TEST BUG#1
  test('should deny access if token is passed in through query', async () => {
    const response = await request(app)
    .get(`/users?_token=${tokens.u1}`)
  expect(response.statusCode).toBe(401);
  })
});
...
describe("PATCH /users/[username]", function() {
  ...
  // BUG #3
  test("should not include hashed PW in response", async () => {
    const response = await request(app)
    .patch("/users/u1")
    .send({ _token: tokens.u3, first_name: "new-fn1" }); // u3 is admin
  expect(response.statusCode).toBe(200);
  ...
  test("should return 404 if cannot find", async function() {
    const response = await request(app)
      ...
      .send({ _token: tokens.u3, password: "new-password" }); // u3 is admin
    expect(response.statusCode).toBe(404);
  });

  /**BUG #6 */
  test("should allow user to update themself without being an admin", async () => {
    const response = await request(app)
    .patch("/users/u1")
